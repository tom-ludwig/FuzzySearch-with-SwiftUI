...
struct FuzzySearchLevenstein {
    ...
    ///   - array: An array of `ViewModel` objects to search within.
    /// - Returns: An array of `ViewModel` objects that match the fuzzy search query, sorted by name score and description score.
    static func search(query: String, in urls: [URLModel]) -> [URLModel] {
        let filteredResult = urls.filter { url -> Bool in
            let nameScore = score(query: query, url: URL(string: url.fileURL)!)
            return nameScore > 0.0
        }
        
        let sortedResult = filteredResult.sorted { url1, url2 -> Bool in
            let nameScore1 = score(query: query, url: URL(string: url1.fileURL)!)
            let nameScore2 = score(query: query, url: URL(string: url2.fileURL)!)
            if nameScore1 > nameScore2 {
                return true
            } else if nameScore1 < nameScore2 {
                return false
    ...
    ///
    /// The Levenshtein distance is a measure of the similarity between two strings, defined as the minimum number of single-character edits (insertions, deletions, or substitutions) required to transform one string into the other. This function uses a dynamic programming approach to compute the Levenshtein distance.
    ///
    /// - Parameters:
    ///   - sourceString: The first string to compare.
    ///   - targetString: The second string to compare.
